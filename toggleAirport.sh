...
function set_airport {
    ...
    echo "Setting airport to $new_status"
...
eth_names=`networksetup -listnetworkserviceorder | sed -En 's/^\(Hardware Port: .*(Plugable|USB\ 10).*, Device: (en.)\)$/\2/p'`
air_name=`networksetup -listnetworkserviceorder | sed -En 's/^\(Hardware Port: (Wi-Fi|AirPort), Device: (en.)\)$/\2/p'`

echo "Ethernet names: $eth_names"
...
echo "Previous ethernet status: $prev_eth_status"
echo "Ethernet status: $eth_status"

# Determine whether ethernet status changed
if [ "$prev_eth_status" != "$eth_status" ]; then
    echo "Ethernet status changed: $prev_eth_status -> $eth_status"
